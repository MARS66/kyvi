<template>
  <div class="app-container my">
    ...
    <el-table ref="tables" v-loading="loading" :data="list" @selection-change="handleSelectionChange" :default-sort="defaultSort" @sort-change="handleSortChange">
      ...
      <el-table-column label="操作类型" align="center" width="120"  prop="businessType">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_oper_type" :value="scope.row.businessType"/>
        </template>
      </el-table-column>
      <el-table-column label="日志编号"  width="120" align="center" prop="operId"/>
      ...
      <el-table-column label="操作内容" align="left" width="360">
        <template slot-scope="scope"  >
          <span>{{ getOperation(scope.row.operParam) }}</span>
        </template>
      </el-table-column>
    </el-table>
  ...
  methods: {
    ...
    getOperation(param) {
      try {
        ...
        if (p.length > 1) return '同步考勤机数据';
        const name=this.userList.find(item=>item.userId===p[0].userId)?.nickName ?? '' 
        return `更改${name} ${p[0].attDate} ${p[0].timeType} 考勤记录为 ${ p[0]?.attType===' ' ? '空': p[0]?.attType }`
      } catch (error) {
        return ''
      }
