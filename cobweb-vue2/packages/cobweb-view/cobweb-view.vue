 ...
 * @LastEditors: Kevin
 * @LastEditTime: 2024-10-30 15:50:07
-->
<template>
  <div class="c-preview">
    <div id="cobweb"></div>
...
import alarm from '../utils/speech';
...
export default {
  ...
  methods: {
    ...
    getDataId(pen) {
      let dataId = pen.form?.find((item) => item.key === 'text')?.dataIds
        ?.dataId;
      if (dataId) return dataId;
      (pen.children || []).forEach((id) => {
        const data = window.cobweb
          .find(id)
          ?.form?.find((item) => item.key === 'text')?.dataIds?.dataId;
        if (data) {
          dataId = data;
          return false;
        }
